...
async def login():
    async with async_session() as session:
        ...
        # email 존재하지 않을 경우
        if not result:
            return jsonify({"error": "존재하지 않는 이메일입니다."}), 400

        # 패스워드 일치 여부 확인
        if not bcrypt.checkpw(encoded_pwd, result.UserPwd.encode("utf-8")):
            return jsonify({"error": "패스워드가 일치하지 않습니다."}), 400
        ...
        if not result_token:
            ...
            payload = {
                ...
                "expiryAt" : (datetime.now() + timedelta(minutes=1)).isoformat()
            }
            ...
            # 생성 토큰한 토큰 DB insert
            new_token = JwtInfo(UserID=result.UserID, AccessToken=token, IssuedAt=datetime.now(), ExpiryAt=datetime.now() + timedelta(hours=1))
        ...
        if result_token.ExpiryAt >= datetime.now() and result_token.ExpiryAt - datetime.now() <= timedelta(minutes=30):
            ...
            if update_token:
                ...
                update_token.IssuedAt = datetime.now()
                update_token.ExpiryAt = datetime.now() + timedelta(minutes=30)
                await session.commit()
        ...
        # 토큰 시간이 만료되었을 경우             
        if result_token.ExpiryAt < datetime.now():
            return jsonify({"error": "토큰 시간이 만료되었습니다. 다시 로그인 해주세요."}), 444
